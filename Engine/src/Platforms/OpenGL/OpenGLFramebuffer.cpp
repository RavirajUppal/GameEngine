...
namespace RealEngine{
    ...
    static GLenum TextureTarget(bool multisampled)
		...
		static GLenum FBTextureFormatToGL(FramebufferTextureFormat format)
		{
			...
			REALENGINE_ASSERT(false, "Unsupported FramebufferTextureFormat");
			return 0;
		}
	...
	{
		for (auto spec : m_Specification.Attachments.Textures)
		{
    ...
    {
        REALENGINE_ASSERT(index < m_ColorAttachments.size(), "invalid index for color attachment"); 
    ...
    void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			REALENGINE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		REALENGINE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	{
		REALENGINE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		REALENGINE_ASSERT(attachmentIndex < m_ColorAttachments.size());
		glBindFramebuffer(GL_FRAMEBUFFER, m_RendererID);
		glClearBufferfv(GL_COLOR, attachmentIndex, (float*)&value);
